...
function rightInput(firstNums, secondNums, firstNumsArr, secondNumsArr) {
    if ((/^,/.test(firstNums)) || (/^,/.test(secondNums)) || (/^0/.test(secondNums))
        || (/,0/.test(secondNums))
        || isNaN(TIME)) {
        alert ('Неверный формат значений.');
        return 0;
    }
...
function inputInInt(arr) {
    for (var i = 0; i < arr.length; i++) {
        if (arr[i].match(/^\d+$/)) {
            ...
            alert("Error in making int array.");
    }
    return arr;
}
...
function checkIntArr(firstNumsArr, secondNumsArr) {
    for (var i = 0; i < firstNumsArr.length; i++) {
        ...
        /*if (firstNumsArr[i] < secondNumsArr[i] && firstNumsArr[i] != 0) {
            alert("Есть пара, где вторая компонента больше");
            return 0;
        }*/
    }
    return 1;
